<?php
    ...
    if ($sw_lng <= $ne_lng)
    {
        ...
        $rows = query("SELECT * FROM heritage_places WHERE ? <= latitude AND latitude <= ? AND (? <= longitude AND longitude <= ?) GROUP BY place_id ORDER BY RAND() LIMIT 50", $sw_lat, $ne_lat, $sw_lng, $ne_lng);
    }
